...
public class Ticket {
    ...
    public void insertIntoTicketTable() {
        ...
        try {
            ...
            String query = "insert into Ticket(PNR_number,Travel_date,Train_number,Train_name,Source,Destination,Total_price) values (?,?,?,?,?,?,?)";
    ...
    public int getLastCounter() {
        ...
        try {
            DBConnection db = new DBConnection();
            Connection connection = db.getConnection();
            String query="SELECT * FROM Ticket WHERE Counter=(SELECT max(Counter) FROM Ticket)";
            PreparedStatement pstmt = connection.prepareStatement(query);
